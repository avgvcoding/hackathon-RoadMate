...
class _TowingServiceScreenState extends State<TowingServiceScreen> {
  ...
  Future<void> _fetchTowingServices() async {
    try {
      ...
      // Prepare API request to fetch nearby towing services.
      ...
      if (response.statusCode == 200) {
        ...
        _towingServices = results.map((json) => Place.fromJson(json)).toList();

        // Add markers for each towing service using an orange marker.
        for (var service in _towingServices) {
          _markers.add(
            Marker(
              ...
              infoWindow: InfoWindow(
                ...
                snippet: service.formattedAddress,
              ),
        ...
        throw Exception('Failed to load towing services: ${response.body}');
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Nearby Towing Services'),
        backgroundColor: const Color.fromARGB(255, 130, 138, 255),
      ),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : ListView.builder(
              itemCount: _towingServices.length + 1,
              itemBuilder: (context, index) {
                if (index < _towingServices.length) {
                  final service = _towingServices[index];
                  return Padding(
                    ...
                    child: Card(
                      ...
                      child: InkWell(
                        borderRadius: BorderRadius.circular(16),
                        onTap: () {
                          _openInGoogleMaps(service.displayName);
                        },
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              // Header row with an icon and towing service title.
                              Row(
                                children: const [
                                  Icon(
                                    Icons.local_shipping,
                                    color: Color.fromARGB(255, 255, 154, 47),
                                    size: 28,
                                  ),
                                  SizedBox(width: 8),
                                  Text(
